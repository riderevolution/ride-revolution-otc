<template>
  <transition name="fade">
    <div class="content" v-if="loaded">
      <div id="admin" class="cms_dashboard">
        <section id="top_content" class="table booker">
          <div class="action_wrapper">
            ...
            <div class="actions">
              <form
                :class="`customer_filter_flex ${
                  $store.state.disableBookerUI ? 'disable_booker' : ''
                }`"
                id="filter"
                @submit.prevent
              >
                ...
                <div
                  class="form_group selection margin"
                ...
                >
                  ...
                  <input
                    ...
                    } ${!findCustomer && customer == '' ? 'highlighted' : ''}`"
                    @click="toggleCustomers = true"
                    @keyup="search($event)"
                    v-model="form.search"
                  />
                  <transition name="slide">
                    <span
                      class="validation_errors alt"
                      v-if="!findCustomer && customer == ''"
                    >
                      Select Customer
                    </span>
                  </transition>
                  <div
                    :class="`customer_selection ${
                      customerLength > 6 ? 'scrollable' : ''
                    }`"
                    v-if="toggleCustomers"
                  >
                    <div class="customer_selection_list">
                      <div
                        class="customer_wrapper"
                        v-if="customerLength > 0 && customer.id != data.id"
                        :id="`customer_${data.id}`"
                        v-for="(data, key) in populateCustomers"
                        :key="key"
                        @click="getCustomer(data)"
                      >
                        <img
                          :src="data.customer_details.images[0].path_resized"
                          v-if="
                            data.customer_details.images[0].path_resized != null
                          "
                        />
                    ...
                    <div class="customer_picked" v-if="customer != ''">
                      <div class="customer_header">
                        <img
                          ...
                          :src="
                            customer.customer_details.images[0].path_resized
                          "
                          v-if="
                            customer.customer_details.images[0].path_resized !=
                            null
                        ...
                        <div class="customer_details">
                          <h2 class="customer_name">
                            <nuxt-link
                              ...
                              @click.native.prevent="
                                openWindow(`/customers/${customer.id}/packages`)
                              "
                            >
                          ...
                          <div class="customer_info">
                            <span>
                              ...
                              {{
                                $moment(
                                  customer.customer_details.co_birthdate
                                ).format('M/D/YY')
                              }}
                            </span>
                            <span>
                              {{ customer.customer_details.co_contact_number }}
                            </span>
                            <span>
                              Store Credit:
                              {{
                                totalItems(
                                  customer.store_credits != null
                                    ? customer.store_credits.amount
                                    : 0
                                )
                              }}
                            </span>
                            <span>
                              ...
                              <b>
                                US -
                                {{ customer.customer_details.co_shoe_size }}
                              </b>
                            </span>
                          </div>
                          <div
                            class="close_wrapper alternate"
                      ...
                      <div class="customer_footer" v-if="customer != ''">
                        <div class="item" @click="toggleLayout('attendance')">
                          Attendance
                        </div>
                        <div class="item" @click="toggleLayout('package')">
                          Packages
                        </div>
                        <div class="item" @click="toggleLayout('redeem')">
                          Redeem
                        </div>
                        <div
                          id="credits"
                          class="item"
                          @click="toggleQuickSale('credit')"
                        >
                          Buy Credits
                          <transition name="slide">
                            <span
                        ...
                        <div class="item" @click="toggleQuickSale('product')">
                          Buy Products
                        </div>
                      </div>
                    </div>
          ...
          <div class="booker_wrapper">
            ...
            >
              <div class="header_wrapper">
                  ...
                  <div class="booker_prev" @click="generatePrevClasses()">
                    ...
                    >
                      <g transform="translate(-248 -187)">
                        <g class="arrow_1" transform="translate(248 187)">
                          <circle class="arrow_3" cx="14" cy="14" r="14" />
                          <circle class="arrow_4" cx="14" cy="14" r="13.5" />
                        </g>
                        <path
                          class="arrow_2"
                          d="M184.939,200.506l-3.981,3.981,3.981,3.981"
                          transform="translate(445.438 405.969) rotate(180)"
                        />
                      </g>
                    </svg>
                  </div>
                ...
                <div>
                  <div class="action_calendar_btn" @click="populateClasses()">
                    Today
                  </div>
                  <div class="action_btn alternate" @click="hideClass('hide')">
                    Hide Classes
                  </div>
                </div>
              </div>
              <div class="content_wrapper">
                <div
                  ...
                  v-for="(result, key) in populateDates"
                  :key="key"
                >
                  <div
                    ...
                    @click.self="
                      toggleClass(
                        $event,
                        $moment(result.date).format('M'),
                        $moment(result.date).format('D'),
                        $moment(result.date).format('YYYY'),
                        result,
                        key
                      )
                    "
                  >
                    {{ result.abbr }} | {{ result.date }}
                  </div>
                  <div class="accordion_content">
                    <div
                      :id="`class_${dkey}_${key}`"
                      :class="[
                        'class_content',
                        data.schedule.studio.online_class ? 'online' : '',
                        data.highlighted ? 'highlighted' : ''
                      ]"
                      v-for="(data, dkey) in schedules"
                      :key="dkey"
                      @click="getBookings(data, dkey, key)"
                    >
                      <div class="class_title">
                        <span>
                          {{ data.schedule.start_time }},
                          {{
                            data.schedule.custom_name != null
                          ...
                          <img
                            src="/icons/private-class.svg"
                            v-if="data.schedule.private_class == 1"
                          />
                        </span>
                        <div
                          :class="`class_status ${data.isFull ? 'full' : ''}`"
                        >
                          {{
                            data.isFull
                              ? `Full (${data.schedule.studio.seats.length})`
                      ...
                      <div
                        class="class_text"
                        v-html="instructorLabel(data, 'violator')"
                      ></div>
            ...
            <div class="booker_button" v-if="hide_class">
              <div class="action_btn alternate" @click="hideClass('show')">
                Show Classes
              </div>
            </div>
              ...
              <div class="booker_seats">
                <div class="seat_controls">
                  <div class="left_side">
                    <div class="class_options">
                      <select
                        :class="`default_select alternate ${
                          schedule != '' ? '' : 'disable_booker'
                        } ${
                          $store.state.disableBookerUI ? 'disable_booker' : ''
                        }`"
                        name="class_options"
                        @change="getClassOptions($event)"
                        v-if="!studio.online_class"
                      >
                      ...
                      >
                        ...
                        <option
                          :value="classOption"
                          v-for="(classOption, key) in classOnlineOptions"
                          :key="key"
                        >
                          {{ classOption }}
                        </option>
                      ...
                      <div class="class_info" v-show="!studio.online_class">
                        <div
                          class="action_calendar_btn"
                          id="legend_toggler"
                          @click="toggleLegends($event)"
                        ...
                        <div class="overlay">
                          ...
                          <div class="type_content">
                            <div
                              class="type"
                              v-for="(data, key) in customerTypes"
                              :key="key"
                            >
                    ...
                    <label
                      class="booker_label"
                      v-html="instructorLabel(schedule, 'title')"
                    ></label>
                    <div
                      :class="`controls ${studio.online_class ? 'nope' : ''}`"
                    >
                  ...
                  <div
                    :class="`right_side ${studio.online_class ? 'nope' : ''} ${
                      $store.state.disableBookerUI ? 'disable_booker' : ''
                    }`"
                  >
                ...
                <div v-if="!studio.online_class">
                  ...
                  <download-csv
                    ...
                    :name="`studio-attendance-${$moment(schedule.date).format(
                      'MM-DD-YY'
                    )}-${$moment(schedule.schedule.start_time, 'h:mm A').format(
                      'h-mm-A'
                    )}.csv`"
                  >
                    Export
                  ...
                  >
                    <seat-plan
                      ref="plan"
                      :customer="customer"
                      :onlineClass="studio.online_class ? true : false"
                    />
                  </panZoom>
                </div>
                ...
                <online-attendance-layout
                  ref="online"
                  v-else-if="studio.online_class"
                  :schedule="schedule"
                  :studio="studio"
                />

                <div class="seat_legends" v-if="!studio.online_class">
                  <div class="legend_title gray">
                    <span></span>
                    Booked
                  </div>
                  <div class="legend_title margin green">
                    <span></span>
                    Signed In
                  </div>
                  <div class="legend_title margin white">
                    <span></span>
                    Available
                  </div>
                  <div class="legend_title margin gradient">
                    <span></span>
                    Blocked/Comp
                  </div>
                  <div
                    ...
                    v-if="$store.state.disableBookerUI"
                  >
                    Cancel
                  </div>
                </div>
                ...
                <div class="booker_notepad">
                  ...
                  <div class="notepad_text">
                    <textarea
                      ...
                      rows="10"
                      v-model="notePad"
                      @focusout="updateNotes($event)"
                    ></textarea>
                  </div>
                </div>
                <div
                  :class="`booker_waitlist ${
                    studio.online_class ? 'nope' : ''
                  }`"
                >
                  <div class="footer_header">
                    ...
                    <div
                      :class="`action_success_btn ${
                        inWaitlist ||
                        $store.state.customerID == 0 ||
                        $store.state.scheduleID == 0 ||
                        (waitlists.length > 0 && waitlists[0].past == 1)
                          ? 'disabled'
                          : ''
                      }`"
                      @click="addToWaitlist()"
                    ...
                    <tbody v-if="waitlists.length > 0">
                      <tr v-for="(waitlist, key) in waitlists" :key="key">
                        ...
                        <td>
                          <div class="action">
                            <div
                              @click="prioritizeWaitlist(waitlist)"
                              :class="`link ${
                                waitlist.past == 1 ? 'disabled' : ''
                              }`"
                            >
                              Book Now
                            </div>
                            <div
                              :class="`link margin ${
                                waitlist.past == 1 ? 'disabled' : 'cancel'
                              }`"
                              @click="removeToWaitlist(waitlist.id)"
                            >
                              Remove
                            </div>
                          </div>
      ...
      <transition name="fade">
        <prompt
          v-if="$store.state.promptStatus"
          :message="$refs.plan.hasCancel ? $refs.plan.message : message"
          :hasCancel="$refs.plan.hasCancel"
        />
      </transition>
      <transition name="fade">
        <prompt-broken-bike
          v-if="$store.state.promptBrokenBikeStatus"
          :message="brokenMessage"
        />
      </transition>
      <transition name="fade">
        <prompt-booker
          v-if="$store.state.promptBookerStatus"
          :message="$refs.plan ? $refs.plan.message : plan.message"
        />
      </transition>
      <transition name="fade">
        <prompt-booker-action
          v-if="$store.state.promptBookerActionStatus"
      ...
      <transition name="fade">
        <prompt-waitlist
          v-if="$store.state.promptWaitlistStatus"
          :value="waitlistID"
        />
      </transition>
      <transition name="fade">
        <assign
          v-if="$store.state.assignStatus"
          :type="$refs.plan.assignType"
        />
      </transition>
      <transition name="fade">
      ...
      <transition name="fade">
        <customer-package
          v-if="$store.state.customerPackageStatus"
          :studioID="studioID"
          :type="packageMethod"
        />
      </transition>
      ...
      <transition name="fade">
        <attendance-layout
          v-if="$store.state.attendanceLayoutStatus"
          :customer="customer"
        />
      </transition>
      <transition name="fade">
        <package-layout
          v-if="$store.state.packageLayoutStatus"
          :customer="customer"
        />
      </transition>
      <transition name="fade">
        <online-attendance-customer
          v-if="$store.state.onlineAttendanceCustomer"
          :schedule="schedule"
        />
      </transition>
      <transition name="fade">
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        classOptions: [
          'Cancel Class',
          'Print Sign-in Sheet w/ Measurements',
          'Print Room'
        ],
        classOnlineOptions: [
          'Cancel Class',
        ...
        for (let i = 0, i_len = me.results.length; i < i_len; i++) {
          if (me.customer != '') {
            if (me.customer.bookings.length > 0) {
              for (
                let j = 0, j_len = me.customer.bookings.length;
                j < j_len;
                j++
              ) {
                if (
                  me.$moment(me.results[i].date).format('YYYY-MM-DD') ==
                  me.customer.bookings[j].scheduled_date.date
                ) {
    ...
    methods: {
      ...
      getClasses() {
        ...
        me.$axios
          .get(
            `api/studio-class-bookings?scheduled_date_id=${me.scheduledDateID}&schedule_id=${me.schedule.schedule.id}`
          )
          .then(res => {
            if (res.data) {
      ...
      getClassOptions(event) {
        ...
        switch (target) {
          ...
          case 'Print Room':
            window.open(
              `${window.location.origin}/print/booker/room?scheduled_date_id=${me.scheduledDateID}&studio_id=${me.studio.id}`,
              '_blank'
            )
            break
          case 'Print Sign-in Sheet w/ Measurements':
            if (me.studio.online_class) {
              window.open(
                `${window.location.origin}/print/booker/online-attendance?scheduled_date_id=${me.scheduledDateID}&studio_id=${me.studio.id}`,
                '_blank'
              )
            } else {
              window.open(
                `${window.location.origin}/print/booker/attendance?scheduled_date_id=${me.scheduledDateID}&studio_id=${me.studio.id}`,
                '_blank'
              )
            }
            break
        ...
        setTimeout(() => {
          ...
          if (!picked) {
            ...
            me.$store.state.errorList = [
              'No available seat found in this schedule.'
            ]
          ...
          me.$axios
            ...
            .then(res => {
              if (res.data.result == 0) {
                ...
                setTimeout(() => {
                  if (me.$refs.plan) {
                    me.$refs.plan.message =
                      'The customer has already been scheduled.'
                  }
                }, 10)
      ...
      noShow() {
        ...
        formData.append(
          'type',
          me.$store.state.seat.status == 'comp' ? 'comp' : 'booking'
        )
        formData.append(
          'data_id',
          me.$store.state.seat.comp.length > 0
            ? me.$store.state.seat.comp[0].id
            : me.$store.state.seat.bookings[0].id
        )
        me.loader(true)
        me.$axios
          .post('api/bookings/no-show', formData, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          })
      ...
      removeAssign() {
        ...
        if (me.$store.state.compID != 0) {
          me.loader(true)
          me.$axios
            .delete(`api/comp/${me.$store.state.compID}`, {
              headers: {
                Authorization: `Bearer ${token}`
              }
            })
      ...
      blockBike() {
        ...
        if (me.$store.state.seat != '') {
          ...
          me.$axios
            .get(
              `api/seats/${me.$store.state.seat.id}?scheduled_date_id=${me.scheduledDateID}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`
                }
            ...
            .then(res => {
              if (res.data) {
                ...
                me.$axios
                  .post(
                    `api/seats/update-status/${me.$store.state.seat.id}`,
                    formData,
                    {
                      headers: {
                        Authorization: `Bearer ${token}`
      ...
      getBookings(data, sunique, unique) {
        ...
        if (me.studioID) {
          let element = document.getElementById(`class_${sunique}_${unique}`),
            parents = document.querySelectorAll(
              '.booker_classes .content_wrapper .class_accordion'
            )
          parents.forEach((parent, pindex) => {
            let children = parent.querySelectorAll(
              '.accordion_content .class_content'
            )
          ...
          setTimeout(() => {
            if (me.$refs.plan) {
              ...
              document.querySelector(
                '.plan_wrapper'
              ).style.transform = `matrix(0.4, 0, 0, 0.4, ${me.customWidth}, ${me.customHeight})`
            }
          }, 10)
        } else {
        ...
        let formData = new FormData(),
          ...
          id = me.$store.state.user.id

        formData.append('scheduled_date_id', data.id)
        formData.append('user_id', id)
        me.$axios
          .post('api/schedule-notes', formData, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          })
      ...
      fetchWaitlist(schedule_id) {
        ...
        let ctr = 0
        me.$axios
          .get(`api/waitlists?scheduled_date_id=${schedule_id}`)
          .then(res => {
            if (res.data) {
      ...
      toggleOverlays(e) {
        const me = this
        let target = e.target,
          element = document.getElementById(`legend_toggler`)
        if (element !== target) {
          ...
          if (me.customer.bookings) {
            for (let i = 0, i_len = me.schedules.length; i < i_len; i++) {
              for (
                ...
                if (
                  me.schedules[i].id ==
                  me.customer.bookings[j].scheduled_date_id
                ) {
      ...
      searchCustomer(data) {
        ...
        me.$axios
          .post('api/customers/search', formData)
          .then(res => {
            if (res.data) {
      ...
      getStudio(event) {
        ...
        document
          .querySelectorAll('.content_wrapper .class_accordion')
          .forEach((value, index) => {
      ...
      updateNotes(event) {
        ...
        me.$axios
          .post('api/extras/update-user-notepad', formData, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          })
      ...
      panZoomInit(instance, id) {
        ...
        let planWidth = document
          .querySelector(
            '.booker_wrapper .booker_content .booker_seats .vue-pan-zoom-scene'
          )
          .getBoundingClientRect().width
        let planHeight = document
          .querySelector('.plan_wrapper')
        ...
        document
          ...
          .addEventListener('click', function (e) {
            ...
            document.querySelector(
              '.plan_wrapper'
            ).style.transform = `matrix(0.4, 0, 0, 0.4, ${planWidth}, ${planHeight})`
          })
        document
      ...
      customZoom(instance, type) {
        ...
        switch (type) {
          case 'in':
            ...
            instance.smoothZoom(
              planWidth / 2 + me.zoomCtr,
              planHeight / 2 + me.zoomCtr,
              me.zoomCtr
            )
            break
          case 'out':
      ...
      generateNextClasses() {
        ...
        for (let i = 0; i < 7; i++) {
          if (
            me.current >
            me
              .$moment(`${me.currentYear}-${me.currentMonth}`, 'YYYY-MM')
              .daysInMonth()
          ) {
            me.current =
              me.current -
              ...
              if (me.current <= 0) {
                if (me.last <= 0) {
                  ...
                  if (me.currentMonth == 12) {
                    me.current = me
                      .$moment(
                        `${me.currentYear}-${me.currentMonth}-1`,
                        'YYYY-MM'
                      )
                      .daysInMonth()
                  } else {
                ...
                me.current = me
                  .$moment(
                    `${me.currentYear}-${me.currentMonth}-${me.last}`,
                    'YYYY-MM-D'
                  )
                  .daysInMonth()
              }
        ...
        for (let i = 0; i < 7; i++) {
          ...
          me.results.push({
            abbr:
              me
                .$moment(
                  `${me.currentYear}-${me.currentMonth}-${currentDate}`,
                  'YYYY-MM-D'
                )
                .format('M-D') == me.$moment().format('M-D')
                ? 'Today'
                : me
                    .$moment(
                      `${me.currentYear}-${me.currentMonth}-${currentDate}`,
      ...
      populateResults(data, type) {
        ...
        for (let i = 0, len = me.results.length; i < len; i++) {
          ...
          document
            .getElementById(`sched_${i}`)
            .querySelector('.accordion_content').style.height = 0
        }
          ...
          case 'next':
            me.results.push({
              abbr:
                me
                  .$moment(
                    `${me.currentYear}-${me.currentMonth}-${data}`,
                    'YYYY-MM-D'
                  )
                  .format('M-D') == me.$moment().format('M-D')
                  ? 'Today'
      ...
      async toggleClass(event, month, day, year, data, unique) {
        ...
        if (!data.toggled) {
          ...
          await me.$axios
            .get(
              `api/schedules?month=${month}&year=${year}&day=${day}&studio_id=${me.studioID}&for_booker=1`
            )
            .then(res => {
              if (res.data) {
                ...
                if (me.customer != '') {
                  if (me.customer.bookings) {
                    for (
                      let i = 0, i_len = me.schedules.length;
                      i < i_len;
                      i++
                    ) {
                      for (
                ...
                setTimeout(() => {
                  data.toggled = true
                  document
                    .getElementById(`sched_${unique}`)
                    .querySelector('.accordion_content').style.height = `${
                    document
                      .getElementById(`sched_${unique}`)
                      .querySelector('.accordion_content').scrollHeight
                  }px`
                }, 500)
              }
            })
