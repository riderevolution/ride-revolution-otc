    ...
    <div class="content" v-if="loaded">
      <div id="admin" class="cms_dashboard">
        <section id="top_content" class="table">
          <nuxt-link
            to="/reporting/class-report/attendance-by-timeslot"
            class="action_back_btn"
          >
            <img src="/icons/back-icon.svg" />
            <span>Attendance By Timeslot</span>
          </nuxt-link>
          <div class="action_wrapper">
            ...
            <div class="actions">
              ...
              <download-csv
                ...
                :name="`attendance-by-timeslot-${$route.params.slug}-${$moment(
                  form.start_date
        ...
        <section id="content">
          <table class="cms_table alt">
            ...
            <tbody v-if="res.bookings.data.length > 0">
              <tr v-for="(data, key) in res.bookings.data" :key="key">
                ...
                <td>
                  {{
                    $moment(data.scheduled_date.date).format('MMMM DD, YYYY')
                  }}
                </td>
          ...
          <pagination
            :apiRoute="res.bookings.path"
            :current="res.bookings.current_page"
            :last="res.bookings.last_page"
          />
        </section>
      </div>
      <transition name="fade">
  ...
  export default {
    ...
    methods: {
      ...
      getClasses() {
        ...
        me.$axios
          .post(
            `api/exports/class-report/attendance-by-timeslot/${me.$route.params.slug}`,
            formData
          )
          .then(res => {
            if (res.data) {
      ...
      fetchData(type) {
        ...
        me.$axios
          .post(
            `api/reporting/classes/attendance-by-timeslot/${me.$route.params.slug}`,
            formData
          )
          .then(res => {
            setTimeout(() => {
