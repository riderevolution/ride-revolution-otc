        ...
        <div class="content" v-if="loaded">
            <div id="admin" class="cms_dashboard">
                <section id="top_content" class="table">
                    <nuxt-link to="/reporting/sales-report/store-credits-report" class="action_back_btn"><img src="/icons/back-icon.svg"><span>Store Credits Report</span></nuxt-link>
                    <div class="action_wrapper">
                        <div>
                            <div class="header_title">
                                <h1>{{ customer.fullname }} Store Credits Report</h1>
                            </div>
                            <div class="action_btn alternate no_margin" @click="openWindow(`/customers/${customer.id}/packages`)">View Customer</div>
                        </div>
                        <div class="actions">
                            <a :href="`/print/reporting/sales/store-credits-report/${$route.params.slug}/view?start_date=${form.start_date}&end_date=${form.end_date}&studio_id=${form.studio_id}`" target="_blank" class="action_btn alternate">Print</a>
                            ...
                            <download-csv
                                ...
                                class="hidden me"
                                :data="storeCreditsReportSlugAttributes"
                                :name="`store-credits-report-${$route.params.slug}.csv`">
                                Export
                            </download-csv>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            storeCreditsReportSlugAttributes () {
                const me = this
                return [
                    ...me.values.map(value => ({
        ...
        methods: {
            ...
            getSales () {
                ...
                me.loader(true)
                me.$axios.post(`api/reporting/sales/store-credits-report/${me.$route.params.slug}`, formData).then(res => {
                    if (res.data) {
