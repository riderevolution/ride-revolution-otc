    ...
    >
      <div class="modal_wrapper">
        <h2 class="form_title">
          ...
          <br />
          <span>
            ({{ $parent.schedule.schedule.class_credits }} class
            {{
              $parent.schedule.schedule.class_credits <= 1
                ? 'CREDIT'
                : 'CREDITS'
            }}
            will be deducted)
        ...
        <div class="modal_main_group scrollable">
          <div class="form_flex_radio alternate margin new">
            <div
              :id="`pckg_${data.id}`"
              :class="`form_radio ${checkPackage(data, type)}`"
              v-for="(data, key) in res"
              :key="key"
            >
              <input
                type="radio"
                :id="`package_${key}`"
                :checked="
                  type == 'create-waitlist'
                    ? data.id == customer.waitlist.user_package_count_id
                    : notExistingPackage
                    ? data.primary
                    : user_package_count_id == data.id
                "
                :value="data.id"
                name="packages"
                class="action_radio"
          ...
          <div
            class="form_footer_wrapper"
            v-if="!switchPackage && !old_package_count_id"
          >
          ...
          <div
            class="form_footer_wrapper"
            v-else-if="notExistingPackage && switchPackage"
          >
            <div class="button_group">
              <div class="action_cancel_btn" @click="toggleClose()">Cancel</div>
              <button
          ...
          <div
            ...
            v-else-if="
              !notExistingPackage &&
              switchPackage &&
              new_package_count_id != old_package_count_id
            "
          >
            <div class="button_group">
              <div class="action_cancel_btn" @click="toggleClose()">Cancel</div>
  ...
  export default {
        ...
        me.res.forEach((value, index) => {
          if (index == key) {
            ...
            if (document.getElementById(`package_${index}`)) {
              document
                .getElementById(`package_${index}`)
                .parentNode.classList.remove('toggled')
            }
          }
        })
      },
          ...
          if (valid) {
            switch (me.type) {
              ...
              case 'update':
                ...
                formData.append(
                  'new_user_package_count_id',
                  me.new_package_count_id
                )
                formData.append(
                  'old_user_package_count_id',
                  me.old_package_count_id
                )
                me.loader(true)
                me.$axios
                  .post('api/bookings/change-package', formData, {
                    headers: {
                      Authorization: `Bearer ${token}`
                    }
                  })
              ...
              case 'create-guest':
                if (me.user_package_count_id != 0) {
                  ...
                  me.$axios
                    .post(
                      'api/extras/check-if-user-is-booked-already',
                      formData,
                      {
                        headers: {
                          Authorization: `Bearer ${token}`
    ...
    async mounted() {
      ...
      if (me.$store.state.customerID != 0) {
        ...
        me.$axios
          .get(
            `api/customers/${me.$store.state.customerID}/packages?forOTCBooking=1&forWebBooking=1&scheduled_date_id=${me.$parent.schedule.id}`
          )
          .then(res => {
            if (res.data) {
              ...
              setTimeout(() => {
                ...
                if (res.data.customer.user_package_counts.length > 0) {
                      ...
                      if (
                        ...
                        if (parseInt(data.count) > 0) {
                          if (ctr == 0) {
                            if (
                              parseInt(data.count) >=
                              me.$parent.schedule.schedule.class_credits
                            ) {
                              if (data.valid) {
                                me.user_package_count_id = data.id
                                me.new_package_count_id = data.id
                          ...
                          if (
                            me.$store.state.seat &&
                            me.$store.state.seat.bookings.length > 0
                          ) {
                            if (
                              data.id ==
                              me.$store.state.seat.bookings[0]
                                .user_package_count_id
                            ) {
                  ...
                  if (
                    ...
                    if (me.res.length > 0) {
                      me.user_package_count_id =
                        me.$store.state.seat.bookings[0].user_package_count_id
                      me.new_package_count_id =
                        me.$store.state.seat.bookings[0].user_package_count_id
                      me.old_package_count_id =
                        me.$store.state.seat.bookings[0].user_package_count_id
                      ...
                      setTimeout(() => {
                        me.$parent.$refs.plan.message =
                          'Please buy a class package first'
                      }, 10)
