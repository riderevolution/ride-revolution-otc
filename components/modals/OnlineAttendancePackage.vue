    ...
    >
      <div class="modal_wrapper">
        <h2 class="form_title">
          ...
          <br />
          <span>
            ({{ schedule.schedule.class_credits }} class
            {{
              schedule.schedule.class_credits <= 1 ? 'CREDIT' : 'CREDITS'
            }}
            will be deducted)
          </span>
        </h2>
  ...
  export default {
              ...
              me.$axios
                ...
                .then(res => {
                  if (res.data) {
                    ...
                    me.$store.state.onlineAttendancePackage = false
                    setTimeout(() => {
                      me.$parent.$parent.actionMessage =
                        'Succesfully added a customer.'
    ...
    async mounted() {
      ...
      if (me.customer != null) {
        ...
        me.$axios
          .get(
            `api/customers/${me.customer.id}/packages?forOTCBooking=1&forWebBooking=1&scheduled_date_id=${me.schedule.id}`
          )
          .then(res => {
            if (res.data) {
              setTimeout(() => {
                      ...
                      if (
                        ...
                        if (parseInt(data.count) > 0) {
                          if (ctr == 0) {
                            if (
                              parseInt(data.count) >=
                              me.schedule.schedule.class_credits
                            ) {
                              if (data.valid) {
                                me.user_package_count_id = data.id
                                me.new_package_count_id = data.id
